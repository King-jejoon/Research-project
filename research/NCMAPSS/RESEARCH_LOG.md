...
**왜 전이되지 않는가 — 진단 결론 (2026-08-21, `experiments/exp_dsx_why.py`, `exp_dsx_gate_diag.py`, dev 전용)**
- 게이트 전제(저신뢰 점 = 1.6~2.2× 시끄러운 잔차, ρ≈−0.3)는 9개 전부 성립 → 전제가 변별 변수가 아님.
- 변별 변수 = 오차 유형: hs 라벨이 센서 신호보다 앞서는 **라벨 지연**(오라클−hs: DS03 +12[게이트 작동 중거리 유닛 +7~10], DS08a +9, DS07 +13, DS02 +15, DS05/06 +16~18, DS01 +22, DS04 +33). 검출기는 모든 데이터셋에서 오라클보다 3~17사이클 선행 — 검출기 문제 아님. 게이트는 잡음형 오차(DS03 중거리 4유닛)만 고치고 지연형 오차는 못 고침. DS08a/08c 단거리는 저신뢰 점 1~2%라 게이트 자체가 비활성.
- 센서 그리드 아님: 고정 5센서가 데이터셋별 top-5 zRMS 질량 ≥99% 포착(T50·T48이 전부 1·2위). 신호 세기 차이는 있음(T50 zRMS DS03 2.99·DS08a 3.10 vs DS04 1.43).
- V축 120단계 확장: DS08a/08c 0/120, DS05/06 2~3/120 능선(취약), DS03 46/120 — 그리드 확장으로 얻을 것 없음.
- 2단 RUL: 바닥 절반화는 보편, RUL 오차 출처가 다름(DS08a 80% 레벨 말기 붕괴, DS02 test 클래스 이동, DS07 test 난이도). DS03 자체 2단 이득도 p=0.49.
- 제안(미실행): ① 게이트 조건부 발동(기준선 저신뢰 점 비율 < 15%면 off) ② 오라클 대비 온셋 RMSE 병기 ③ 유닛별 초기구간 자기정규화 + detcov 가중 집계 — 전부 dev 스윕으로 먼저 검정(DS03 dev 7.56 유지 확인 후 전이).
