...
SEEDS = [0, 1, 2]
EARLY_HI = 8
RES = os.path.join(HERE, 'm2_validity_results.txt')
...
def m2_of(gp, Wq, Xq, chunk=4096):
    ...
    for i in range(0, len(Wq), chunk):
        ...
        _, _, m2, _, _ = conditional_stats2(gp['model'], gp['lik'], gp['tX'],
                                            gp['tY'], gp['struct'], teX,
                                            m=MCOND, test_y=teY)
        out.append(m2)
...
def main():
    ...
    for sd in SEEDS:
        ...
        for fa, fb, sl in [(F1, F2, slice(half, NCAND)), (F2, F1, slice(0, half))]:
            ...
            m2s[sl] = m2_of(gp, W[pool[fb]], X[pool[fb]])
        fl_m2 = m2s > M2_CUT
        fl_knn = knn_score[cand_i] > KNN_CUT
        both = (fl_m2 & fl_knn).sum()
        log(f'seed{sd}: m2 pct[50,90,99]={np.percentile(m2s,[50,90,99]).round(2)}  '
            f'flags m2={fl_m2.mean():.2%} knn={fl_knn.mean():.2%}  '
            f'overlap={both}/{fl_m2.sum()}m2 {both}/{fl_knn.sum()}knn  '
            f'({time.time()-t0:.0f}s)')
    ...
    for snm in ['in', 'early']:
        ...
        for mi, lab in enumerate(labels):
            v = {n: np.array([R[(sd, n)][snm][mi] for sd in SEEDS])
                 for n in ['base', 'm2clean', 'knnclean', 'rand']}
            dm = v['rand'] - v['m2clean']; dk = v['rand'] - v['knnclean']
            log(f'  {lab:>6}: base {v["base"].mean():.4f} | m2 {v["m2clean"].mean():.4f} '
                f'(vs rand {int((dm>0).sum())}/3, {100*dm.mean()/v["rand"].mean():+.2f}%) | '
                f'knn {v["knnclean"].mean():.4f} '
                f'(vs rand {int((dk>0).sum())}/3, {100*dk.mean()/v["rand"].mean():+.2f}%) | '
                f'rand {v["rand"].mean():.4f}')
