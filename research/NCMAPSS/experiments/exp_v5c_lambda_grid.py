"""
exp_v5c_lambda_grid.py — HI lambda1 x lambda2 grid RE-SWEPT on the
STAGE-2 healthy-range tables (user decision 2026-08-17: the proposed
chain is the final model, so the recipe grid must live on its base).

Protocol byte-identical to exp_v5_lambda_grid / exp_v4_hi2 — same grid,
same frozen loss terms, same composite (Mon + Curv + Range, no Shift),
same shape gate, no med3 inside the grid — ONLY the tables change:
stage-2 ungated healthy-range tables via exp_v5c_dev.tables instead of
the conditional-gate cycle<=3 tables.  Dev only, cached stats, no test
contact.  Note: grid RULs are med3-free by protocol (chain reference
7.56 +/- 0.05 includes med3); they are comparable within the grid only.
Outputs: v5c_lambda_grid_results.txt, v5c_lambda_grid.npz
"""
import os, sys, time
import numpy as np
...
from exp_v5c_dev import tables

L1S = [2.0, 4.0, 6.0, 8.0, 12.0, 16.0]
L2S = [0.25, 0.5, 1.0, 2.0, 4.0]
FROZEN = (12.0, 0.25)
RES = os.path.join(HERE, 'v5c_lambda_grid_results.txt')
...
def main():
    open(RES, 'w').close()
    t00 = time.time()
    log('V5C LAMBDA GRID — stage-2 healthy-range tables (ungated), '
        'composite without Shift, protocol = exp_v5_lambda_grid')
    data = {}
    for sd in SEEDS:
    ...
    if sel != FROZEN:
        log(f'WARNING: winner differs from the frozen recipe {FROZEN} — '
            'chain re-freeze decision required, none taken here.')
    else:
        log(f'frozen recipe {FROZEN} CONFIRMED on the stage-2 base.')
    np.savez(os.path.join(HERE, 'v5c_lambda_grid.npz'),
