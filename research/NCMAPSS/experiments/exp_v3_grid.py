"""
exp_v3_grid.py — STAGE A (full grid): sensor set x kernel x rank under the NEW
data configuration.  One process per sensor set (SET env: 3|5|7).
...
across sets): cycle<3 rows, stratified 1000 per (unit,cycle) -> 18,000 train;
...
Cross-set comparison happens downstream (onset RMSE, Stage A2).
"""
import os, sys, time
import numpy as np
import torch
...
RES = os.path.join(HERE, f'v3_grid_set{SET}_results.txt')
NPZ = os.path.join(HERE, f'v3_grid_set{SET}.npz')
...
def main():
    open(RES, 'w').close()
    t00 = time.time()
    log(f'V3 STAGE A GRID — set{SET} {SENS}  kernels x ranks {[1,2,3]}  (cycle<3, 18k train)')
    cache = L.load_cache()
    ...
    log(f'  {"kernel":>9} | ' + ' | '.join(f'{"rank "+str(r):>16}' for r in [1,2,3]))
    for k in KERNELS:
        cells = []
        for r in [1, 2, 3]:
    ...
    # simplicity rule: among configs within one seed-sd of the best, prefer
    # rbf over matern, then lowest rank
