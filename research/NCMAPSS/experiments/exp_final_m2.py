"""
exp_final_m2.py — FULL pipeline with Stage-1 replaced by the FIXED colleague
condition-likelihood detector (Mahalanobis^2 statistic from demo_cond2, K=3).

  Stage 1  global run-in MOGP + m2-EWMA detection (fixed whitening) -> predicted-normal
  Stage 2  MOGP-Vecchia on predicted-normal                          -> Xhat=f(W)
  Stage 3  per-cycle mean residual (no filter)
  Stage 4  neural HI (MLP + physics loss)
  Stage 5  Bayesian first-passage, truncation RUL at 20/40/60/80 %

Detection is also scored vs ground-truth hs (same as exp_detect_rul).
Compare against exp_detect_rul.py results: detection 80.7% acc, RUL overall
9.17 +/- 0.33 (20%:10.27  40%:5.98  60%:10.70  80%:8.83).

Run:  /opt/anaconda3/envs/pt_prac/bin/python3 exp_final_m2.py
"""
import os, sys, time
import numpy as np
import torch
...
K_M2 = 3.0                 # best operating point from exp_ll_detect
MCOND = 15
RUNIN_PER_UNIT = 250
SEEDS = [0, 1, 2]
RES = os.path.join(HERE, 'final_m2_results.txt')
...
def detect_m2(cache, sd):
    """Stage 1: global run-in MOGP + per-unit m2 EWMA chart (K=3)."""
    W, Xall, A = cache['W_dev'], cache['X_s_dev'], cache['A_dev']
    X = Xall[:, SIDX]
    du = np.unique(A[:, 0].astype(int))
    # global normal model on pooled run-in
    ridx = []
    for u in du:
    ...
    for u in du:
        ...
        UCL = mu0 + K_M2 * sd0 * np.sqrt(LAM / (2 - LAM))
        e = mu0; E = []
        ...
        recs.append(dict(unit=int(u), true=int(tru), pred=int(trans),
...
def run_seed(sd, cache):
    ...
    normal_idx, recs = detect_m2(cache, sd)
...
def main():
    ...
    log(f'FINAL+M2 pipeline  seeds={SEEDS}  NPER={NPER}  K_m2={K_M2}  fracs={FRACS}')
    log(f'reference (T^2 pipeline): detection 80.7% | RUL overall 9.17 +/- 0.33 '
        f'(20%:10.27 40%:5.98 60%:10.70 80%:8.83)')
    ...
    for sd in SEEDS:
        ...
        log(f'  seed{sd}: det acc = {a:.1f}%  transitions = {[r["pred"] for r in recs]}')
        log(f'  seed{sd}: RUL RMSE = {rmse:.2f}  (by trunc {pf})  ({time.time()-t0:.0f}s)')

    flat = [r for recs in all_recs for r in recs]
    log('\n' + '=' * 74)
    log(f'DETECTION (m2, K={K_M2}): acc = {np.mean([r["acc"] for r in flat])*100:.1f}%  '
