"""
exp_v4_test.py — v4 chain: ONE-TIME verification on the sealed DS03 test
units.  (For the DS03 test split as a dataset this is the 4th opening —
1 old cycle<5 chain + 2 v3 chain + this one; recorded honestly in the paper.)

Everything below was frozen on dev BEFORE this run:
  model    : 5 sensors [T30,T48,T50,Nc,Wf], RBF rank1, cycle<=3,
             1000/(unit,cycle) = 27k (dev only; training indices identical to
             v4_c3_train_s{seed}.npz)
  gate     : detcov >= V* = 20.2 — ABSOLUTE dev value, shared by every seed,
  ...
  HI       : trim25 of gate-passing residuals, z-normalized with DEV pooled
             mu/sigma, MLP lambda1=8, lambda2=0.5, end_target=1.03, others
             frozen (lambda0=1, thr 0.2, flat 800/0.004, 1e-3 x 1000ep),
...
Outputs: v4_test_results.txt, v4_test_stats_s{seed}.npz
"""
import os, sys, time
import numpy as np
import torch
...
import exp_lib as L
from exp_v4_c3_detcov import fit, point_stats, SIDX
...
SEEDS = [0, 1, 2]
NPER = 200
VSTAR = 20.2                      # absolute, frozen on dev
L1, L2 = 8.0, 0.5
RES = os.path.join(HERE, 'v4_test_results.txt')
...
def build_test_stats(sd, gp):
    path = os.path.join(HERE, f'v4_test_stats_s{sd}.npz')
...
def main():
    open(RES, 'w').close()
    t00 = time.time()
    log('V4 ONE-TIME TEST VERIFICATION (frozen chain; DS03 test 4th opening '
    ...
    for sd in SEEDS:
        t0 = time.time()
        z = np.load(os.path.join(HERE, f'v4_c3_train_s{sd}.npz'))
        gp, _ = fit(W[z['train_idx']], X[z['train_idx']])
        log(f'seed{sd}: model rebuilt ({time.time()-t0:.0f}s)')

        # dev side (cached stats), gate at the frozen absolute V*
        ...
        t_raw, t_hrs, t_llq, t_ucyc, t_onset, t_dur = summaries(
            Zt, VSTAR, t_units)
        log(f'seed{sd}: test stats ready, V*={VSTAR} (absolute, frozen on dev)')
