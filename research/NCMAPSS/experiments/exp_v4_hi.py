"""
exp_v4_hi.py — v4 STAGE 6: HI network — lambda1 (monotonicity) x lambda2
(convexity) grid on the selected v4 chain (set5 rbf r1, cycle<=3, 27k),
justified TABLE-IV style by HI property metrics.

Gate: ABSOLUTE V* = 20.2 for every seed (user decision from Stage 5: sweep
minimum, sole member of the tie band, 3/3 seed wins, collapse begins at 20.3).
Frozen (unchanged from the chain family): lambda0=1, thr=0.2, end_target=1.03,
flat=(800, 0.004), Adam 1e-3 x 1000 epochs.
Tuned: lambda1 in {2,4,8,16} x lambda2 in {0.5,1,2,4}.
Inputs: per-flight trim25 of gate-passing residuals, dev z-normalized, 3 seeds.
Selection: shape gate (start<=0.30, end>=0.93, tail monotone) 3/3, then
property composite (Mon, Curv, Range, Shift); LOO RUL column as sanity check.
Outputs: v4_hi_results.txt, v4_hi.npz, fig_v4_hi_curves.png
"""
import os, sys, time
import numpy as np
import matplotlib
...
from exp_v4_setcmp import load as load_stats, SEEDS, FILES

SET = 5
VSTAR = 20.2                      # absolute, shared by every seed
L1S = [2.0, 4.0, 8.0, 16.0]
L2S = [0.5, 1.0, 2.0, 4.0]
RES = os.path.join(HERE, 'v4_hi_results.txt')
...
def main():
    open(RES, 'w').close()
    t00 = time.time()
    log(f'V4 STAGE 6 — HI lambda1 x lambda2 grid  '
        f'(set{SET}, absolute gate V*={VSTAR}, trim25)')
    log('  frozen: lambda0=1, thr=0.2, end_target=1.03, flat=(800,0.004), '
        '1e-3x1000ep')
    ...
    log('')
    log('TABLE (rows lambda1; column groups lambda2; cells: '
        'Mon | Curv | Range | Shift)')
    log('  l1\\l2 |' + '|'.join(f'   {l2:^37}' for l2 in L2S))
    for l1 in L1S:
        cells = []
        for l2 in L2S:
            r = R[(l1, l2)]
    ...
    fig.tight_layout()
    p_fig = os.path.join(OUT, 'fig_v4_hi_curves.png')
    fig.savefig(p_fig, dpi=150); plt.close(fig)
    log(f'figure: {p_fig}')
    np.savez(os.path.join(HERE, 'v4_hi.npz'),
