"""
exp_v4_stats.py — v4 STAGE 3: per-point residual statistics for the per-set
winning model under the cycle<=3 window (27k train).

SET env 3|7 (set5's winner rbf rank1 is the pilot configuration —
v4_c3_stats_s{seed}.npz is reused, identical draw and internals).
Winner per set (Stage 2 + 2b, complete-config rule): rbf rank1 for every set.
Per seed: rebuild the winner on the shared stratified split (cycle<=3,
1000/(unit,cycle) = 27k, same rng as the grid), then for every dev unit and
cycle sample 200 rows (rng unit*7+seed) and cache per-point residual /
detcov / corrected LL and the hs onset.
Output: v4_stats_set{SET}_s{seed}.npz
"""
import os, sys, time
import numpy as np
import torch
...
SEEDS = [0, 1, 2]
RES = os.path.join(HERE, f'v4_stats_set{SET}_results.txt')
...
def main():
    open(RES, 'w').close()
    t00 = time.time()
    log(f'V4 STATS — set{SET} {SENS}, {KERNEL} rank{RANK}, '
        f'cycle<={TRAIN_CYC_MAX} stratified 27k')
    cache = L.load_cache()
    ...
    for sd in SEEDS:
        out_path = os.path.join(HERE, f'v4_stats_set{SET}_s{sd}.npz')
